...
export default function UploadDialog({ onClose, onSubmit }: UploadDialogProps) {
    ...
    // 虽然变量名叫 images 保持接口一致，但现在里面也包含视频了
    ...
    return (
        <DialogModal open onClose={handleClose} className='card w-md max-sm:w-full'>
            <div className='space-y-4'>
                <h2 className='text-xl font-bold'>上传内容</h2>

                <div>
                    <label className='text-secondary mb-2 block text-sm font-medium'>选择图片或视频（可多选）</label>
                    <input 
                        ref={fileInputRef} 
                        type='file' 
                    ...
                    {images.length === 0 ? (
                        <div
                            ...
                            <div className='text-center'>
                                ...
                                <p className='text-secondary text-xs'>点击选择内容</p>
                            </div>
                        </div>
                    ) : (
                        <>
                            <div className='relative flex h-40 items-center justify-center overflow-visible rounded-xl bg-linear-to-br from-gray-50 to-gray-100'>
                                ...
                                {images.length > 3 && (
                                    <div className='absolute right-4 -bottom-2 rounded-full bg-black/70 px-3 py-1 text-xs text-white shadow-lg'>共 {images.length} 个</div>
                                )}
                            </div>

                            <div className='mt-3 flex items-center justify-between'>
                                <span className='text-secondary text-xs'>已选择 {images.length} 个文件</span>
                                <button
                                    type='button'
                                    onClick={() => fileInputRef.current?.click()}
                ...
                <div>
                    <label className='text-secondary mb-2 block text-sm font-medium'>描述（可选，应用于本次所有文件）</label>
                    <textarea
                        value={description}
